...
const items = [
  { key: 1, label: (<NavLink className="nav-link" to="/home">Home</NavLink>) },
  ...
  { key: 4, label: (<NavLink className="nav-link" to="/arcade">Arcade</NavLink>) },
  { key: 5, label: (<NavLink className="nav-link" to="/">???</NavLink>) },
  {
    key: 6, label: (
...
export const Layout = ({ children, style }: { children: React.JSX.Element, style?: CSSProperties }) => {
  ...
  return (
    <AntdLayout>
      ...
      <Content>
        <animated.div
          id='gradient'
          style={{ ...style, backgroundImage: allStops.to((...args) => `linear-gradient(${angle}deg, ${args.join(', ')})`) }}
        >
          {children}
        </animated.div>
      </Content>
